...
class CoCurriculumPage extends StatefulWidget {
  ...
  const CoCurriculumPage({super.key, required this.email, required this.name});
...
class _CoCurriculumPageState extends State<CoCurriculumPage> {
  ...
  void mapsToClaimStatus(BuildContext context) {
    Navigator.push(
      ...
      MaterialPageRoute(
        builder: (context) =>
            ClaimStatusPage(student_id: studentId, full_name: studentName),
      ),
    );
  }
  ...
  void mapsToRegisterModule(BuildContext context) {
    Navigator.push(
      ...
      MaterialPageRoute(
        builder: (context) =>
            RegisterModulePage(student_id: studentId, full_name: studentName),
      ),
    ).then((value) => fetchCoCurriculumRecords());
  }
              ...
              return SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                    ...
                    _infoRow(
                      'full_name',
                      readProfileValue(['full_name'], studentName),
                    ),
                    _infoRow(
                      'student_email',
                      readProfileValue(['student_email'], studentId),
                    ),
                    _infoRow(
                      'program_code',
                      readProfileValue(['program_code'], '-'),
                    ),
                    _infoRow(
                      'program_name',
                      readProfileValue(['program_name'], '-'),
                    ),
                    _infoRow(
                      'faculty',
                      readProfileValue(['faculty', 'Faculty'], '-'),
                    ),
                    _infoRow(
                      'current_sem',
                      readProfileValue(['current_sem'], '-'),
                    ),
                    _infoRow(
                      'co_curriculum_credit',
                      readProfileValue(['co_curriculum_credit'], '0'),
                    ),
                  ],
                ),
              );
