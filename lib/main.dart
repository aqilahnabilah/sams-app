    ...
    return MaterialApp(
      ...
      routes: {
        ...
        '/student/attendance-history': (context) =>
            const AuthWrapper(target: 'attendance-history'),
        '/lecturer/attendance-history': (context) =>
            const AuthWrapper(target: 'attendance-history'),
        '/student/attendance-status': (context) =>
            const AuthWrapper(target: 'attendance-status'),
        '/lecturer/sessions': (context) =>
            const AuthWrapper(target: 'lecturer-sessions'),
        '/lecturer/attendance-records': (context) =>
            const AuthWrapper(target: 'attendance-records'),
  ...
  Widget build(BuildContext context) {
    ...
    switch (user.role) {
      case UserModel.roleStudent:
        return StudentDashboard(userId: user.userId, name: user.username);

      case UserModel.roleLecturer:
        return LecturerDashboard(userId: user.userId, name: user.username);

      case UserModel.roleFacultyRegistrar:
        return RegistrarDashboard(userId: user.userId, name: user.username);

      case UserModel.roleTreasury:
        return TreasuryDashboard(userId: user.userId, name: user.username);

      case UserModel.rolePusatAdab:
        return PusatAdabDashboard(userId: user.userId, name: user.username);
