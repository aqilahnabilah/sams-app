...
/// [PaymentController] manages the logic for student fees and payment processing.
/// It handles data fetching from Firestore, file conversion, and state management.
class PaymentController extends ChangeNotifier {
  final FirebaseFirestore _firestore = FirebaseFirestore.instance;

  /// Holds the current student's metadata.
  StudentModel? student;
  
  /// Holds the tuition fee record for the student.
  StudentFeeModel? fee;
  
  /// Holds a specific payment transaction being viewed or edited.
  PaymentModel? payment;
  
  /// UI indicator for background operations.
  bool isLoading = false;
  
  /// Error message to be displayed in the UI if an operation fails.
  String? errorMessage;

  /// Theoretical start date of the semester to calculate the current week.
  static final DateTime semesterStartDate = DateTime.now().subtract(const Duration(days: 35));

  /// Calculates the current academic week based on [semesterStartDate].
  int get currentWeek {
  ...
  /// List of payment modes accepted by the institution.
  static const List<String> supportedPaymentMethods = ['Bank Transfer', 'QR Pay', 'Online Banking'];

  /// Fetches fee and student details from Firestore. 
  /// If no records exist, it initializes default data (Mock registration).
  ...
  /// Converts a physical file (Image/PDF) into a Base64 string for Firestore storage.
  /// Enforces a 1MB size limit to optimize Firestore performance.
  ...
  /// Reconstructs a file from a Base64 string and opens it using the system default app.
  Future<void> saveAndOpenFile(String base64String, String fileName) async {
    try {
      ...
      // Sniff file headers for extension detection
  ...
  /// Creates a new payment submission record in the 'payments' collection.
  ...
  /// Retrieves all payments submitted by a specific student.
  ...
  /// Fetches a specific payment by its invoice number.
  ...
  /// (Treasury Only) Retrieves all payment submissions across all students.
  Future<List<PaymentModel>> getAllPayments() async {
    final snapshot = await _firestore.collection('payments').get();
    return snapshot.docs.map((doc) => PaymentModel.fromFirestore(doc.data())).toList();
  }

  /// (Treasury Only) Updates the status of a payment (Approved/Rejected).
  /// If approved, it automatically triggers a balance sync with the student's fee record.
  ...
  /// Private helper to update the tuition fee record and block status after a payment is approved.
  Future<void> _syncFee(PaymentModel p) async {
    ...
    // Auto-update block status based on week 5 policy
    await _firestore.collection('students').doc(f.studentId).update({'is_blocked': (f.totalAmount - newPaid) > 0 && currentWeek >= 5});
  }

  /// Debug utility to populate campus locations and check-in session flags.
  Future<bool> uploadDataToFirestore() async {
    isLoading = true;
    notifyListeners();
    try {
      final locations = [
        {
          'location_id': 'LOC_PEKAN',
          'campus_name': 'UMPSA Pekan',
          'center_latitude': 3.5437,
          'center_longitude': 103.4288,
          'allowed_meter': 1500.0,
          'is_active': true,
        },
        {
          'location_id': 'LOC_GAMBANG',
          'campus_name': 'UMPSA Gambang',
          'center_latitude': 3.7169,
          'center_longitude': 103.1232,
          'allowed_meter': 1000.0,
          'is_active': true,
        },
      ];

      for (var loc in locations) {
        await _firestore.collection('locations').doc(loc['location_id'] as String).set(loc);
      }

      final sessions = await _firestore.collection('class_sessions').get();
      for (var doc in sessions.docs) {
        if (!doc.data().containsKey('requires_location')) {
          await doc.reference.update({'requires_location': true});
        }
      }
